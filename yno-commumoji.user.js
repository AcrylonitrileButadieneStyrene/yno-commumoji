...
const cache = unsafeWindow.caches.open("yno-commumoji");
const maxAge = 86400000; // 1 day

async function fetchConfiguration() {
    const cached = await (await cache).match(configUrl);
    ...
    await (await cache).put(configUrl, new Response(text, { status, statusText, headers }));
    updateStatus(date);
    return text;
}
